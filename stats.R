...
long <- 
  ...
  select(
    ...
    event_type, action, passer, receiver, defender, point_elapsed_seconds
  ) %>% 
...
seconds_played_per_player <- 
  long %>% 
  distinct(point_number, tournament, opponent, player, point_elapsed_seconds) %>% 
  group_by(player) %>% 
  summarise(
    sum_time = sum(point_elapsed_seconds)
  ) %>% 
